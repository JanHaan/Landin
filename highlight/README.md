...
`layout(c)`, `layout(optimal)`, `layout(packed, u32)`, packed positions,
encoded unions and `link(symbol: ...)` annotations, and
...
It is language-wide evidence, and it is not needed to test or package the
