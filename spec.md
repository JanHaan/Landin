...
independently enforce the other constituent's tracked metadata origin. This is
the mixed tracked/untracked non-guarantee, not evidence that metadata was
dropped from `over`'s declared sources. The caller remains responsible for
both storages; no field-sensitive origin or provenance mechanism is added.
...
extent. A wrong address, a wrong size, or an address whose slot is currently
unoccupied is rejected and counted. After the same slot is freed and reused
