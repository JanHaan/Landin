...
def construct_evidence():
    ...
    #  Refusals live at the stage that can distinguish the construct.  The
    #  parser owns syntax-only shapes; the checker owns names whose meaning
    ...
    for relative in diagnostics:
        ...
        text = io.open(path, encoding="utf-8").read()
        table = re.search(
            r"function Construct \(Item : Refused_(?:Construct|Use)\)"
            r".*?is \(case Item is(.*?)\)\s+with Post", text, re.S)
        if not table:
            continue
        for one in re.findall(r'"\[(\d{4})\]"', table.group(1)):
            evidence.setdefault(one, set()).add("refused by name")
    return evidence
...
def prototype_evidence_records():
    """The firmware driver has an external CPU/peripheral execution runner.
    ...
    records["firmware/derived-driver"] = (path, fields)
    return records
...
def check_coverage_registers(full_run):
    ...
    if not full_run:
        return []
    out = []
    fixtures = prototype_evidence_records()
    ...
    if guarantees is None:
        ...
        required_guarantees = {
            ...
            "text.conversion", "text.slicing", "text.traversal",
            "float.ieee",
            ...
            "aggregates.fill", "distinct.identity",
            ...
            "functions.anonymous", "control.flow", "control.loops",
            "cleanup.defer",
