...
change also updates `tour.txt` and its affected derived fixtures.
...
- **Resolve**: append the answer under an `## Answer` heading, promote every durable discovery, dependency, decision, or disposition to `ROADMAP.md` (and semantic changes to `tour.txt` plus derived fixtures), set `Status: resolved`, then append a context pointer (gist + link) to the map's Decisions-so-far in `map.md`.
