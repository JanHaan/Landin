...
of the two vendored families — `Nunito Sans` for prose, `MonoLisaCode` for
code — so `--ui` and `--mono` cannot name a face the site does not ship
beside the pages. The `@font-face` rules are read out of each family's own
stylesheet rather than transcribed, because what they carry is eighty
`unicode-range` lists and a mistyped range is not a build failure but a
paragraph quietly set in a fallback.
...
| `fonts/*.woff2` | the browser's font loader, once for the whole site | eighty subsets are 965 KB, and a page carrying even the three an English reader needs would be 140 KB heavier for glyphs the next page would carry again |
...
seconds of invisible text. Which of the eighty subsets a reader fetches is
...
drifts outside the vendored ranges fails the build rather than the page.
