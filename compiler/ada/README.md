...
| `Landin.Backend` | where a routine's cells live, the recursive target extent of one neutral field shape, where direct and depth-one fields sit inside an aggregate datum or slot, and the target-byte replay of scalar, fixed-array and unfolded variant runs | name a machine, choose a register, or ask the host a width |
...
construction, inspection and whole copies, and scalar field reads and writes
through one nested ordinary child. That is every opcode `Landin.IR`
