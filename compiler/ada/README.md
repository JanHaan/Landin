...
| `Landin.Tokens.Text` | D161's one UTF-8 and escape decoder shared by lexing, checking and lowering | diagnose, choose a literal context, or own emitted storage |
...
| `Landin.Syntax.Parser` | the parse, including contextual separation of a final `try` expression from a `try` statement followed by more body items, and the only construction of a tree | assign a diagnostic code, or read a byte |
...
| `Landin.Backend.X86_64` | the assembly text for one target, every register in it, collision-safe whole-program symbols, the hosted entry argument/libc bridge, D161's read-only literal data, and the target-width scalar, finite-array, compact repetition, nested-child and selected-variant directives and padding for recursively written aggregate images | decide a language error mapping, write a file, or run a tool |
...
| `Landin.Stages.Lowering` | the walk that eagerly maps checker nominal, conformance and ready routine-instance identities into deterministic IR order, registers D161's content-pooled text datums, passes hidden evidence tables, builds direct plus used flattened erased tables, lowers selected generic and any concept entries through ordinary indirect calls with injected data, then builds and verifies the IR without creating items for templates or static formals; including caller-owned scalar and shaped control joins, temporary-first aggregate call/control results copied into nested or runtime-addressed destinations, checked computed-element address slots, plus reverse-order cleanup calls on selected exits, and refusing to run on a refused program | own the Unit, work out a scope, derive target layout, synthesize a declaration, or raise a diagnostic |
