...
| `Landin.IR` | the target-neutral instructions: items, slots, blocks, values, an aggregate item's, slot's or measurement's field types, and the only construction of one | hold a scope tree, name a machine, ask a width, or hold an offset |
| `Landin.Backend` | where a routine's cells live, where a field sits inside an aggregate one, and the target layout of a measurement field run, counted in target bytes | name a machine, choose a register, or ask the host a width |
