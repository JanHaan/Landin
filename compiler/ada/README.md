...
| `Landin.IR` | the target-neutral instructions: items, slots, blocks, values, an aggregate item's or slot's scalar fields, a measurement's scalar or compact fixed-array fields, and the only construction of one | hold a scope tree, name a machine, ask a width, or hold an offset |
| `Landin.Backend` | where a routine's cells live, where a scalar field sits inside an aggregate value, and the target layout of a scalar/fixed-array measurement run, counted in target bytes | name a machine, choose a register, or ask the host a width |
