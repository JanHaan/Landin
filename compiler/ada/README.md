...
| `Landin.Backend.X86_64` | the assembly text for one target, every register in it, collision-safe whole-program symbols, the hosted entry argument/libc bridge, and the target-width scalar, finite-array, compact repetition, nested-child and selected-variant directives and padding for recursively written aggregate images | decide a language error mapping, write a file, or run a tool |
