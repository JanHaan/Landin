...
truths, slot traffic, checked and wrapping add and subtract, comparisons,
jumps, branches and returns. It raises `Compiler_Defect` on every other opcode
rather than emitting something plausible. The remaining arithmetic, calls and
module data are not there; [0320]'s shift beyond the width still needs the
guard R1.80 records as its own obligation.
...
return, checked arithmetic, wrapping across signed and unsigned boundaries,
and comparison-driven control flow on the hardware the backend emits for. A
