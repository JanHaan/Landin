...
| `Landin.Syntax.Parser` | the parse, including contextual separation of a final `try` expression from a `try` statement followed by more body items and D185's initialized condition-binding form, and the only construction of a tree | assign a diagnostic code, or read a byte |
...
| `Landin.Stages.Resolution` | the order the trees are walked in through the D139 activity view, including D185's condition-initializer outer scope and guarded-body binding scope | own the resolution table, or a code |
