...
| `Landin.IR` | the target-neutral instructions: items, slots, blocks, values, an aggregate datum or slot's scalar or compact fixed-array fields, a field identity on nested element operations, the same neutral shapes in a separate measurement run, and the only construction of one | hold a scope tree, name a machine, ask a width, or hold an offset |
