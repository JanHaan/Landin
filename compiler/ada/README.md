...
| `Landin.Checking` | what type every runtime node and declaration has, including concrete parameterized-alias application shapes and D136's canonical folded array counts but no template/formal or per-instantiation template metadata; structural atom/error sets, nominal aggregate identity, structural fixed-array and anonymous result shapes, aggregate element identity, recursive first-class target-neutral function signatures on values and aggregate fields, ordered result runs, and scalar/fixed-array/unfolded-variant/recursively nested ordinary runtime layout | decide a rule, execute user code, or ask the host for a width |
...
| `Landin.Stages.Checking` | the three type passes, compile-time-only positional substitution, D136's closed target-independent fixed-expression evaluator, scalar/fixed-array normalization of parameterized aliases, and checking-stage diagnostic order | own a table, a code, execute user code, or choose a target-dependent operator width |
