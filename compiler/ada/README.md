...
| `Landin.IR` | the target-neutral instructions: items, slots, blocks, values, an aggregate datum, slot or measurement's scalar, compact fixed-array and unfolded variant shapes, declaration-order folded aggregate images with compact per-field finite, repeated or hybrid array segments, field identities on nested element operations and compound-storage clears, fills, array copies, variant tag/payload loads, selection, scalar payload writes and whole-part copies | hold a scope tree, name a machine, ask a width, or hold an offset or padding byte |
