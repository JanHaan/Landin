...
| `Landin.Checking` | what type every runtime node and declaration has, including concrete parameterized-alias application shapes and D136's canonical folded array counts but no template/formal or per-instantiation template metadata; opaque checker-owned nominal instance identities separate from source declarations, with every enabled nonparameterized struct represented by its template's canonical empty-actual instance; structural atom/error sets, fixed-array and anonymous result shapes, aggregate element identity, recursive first-class target-neutral function signatures on values and aggregate fields, ordered result runs, and target-dependent scalar/fixed-array/unfolded-variant/recursively nested ordinary runtime layout keyed by target-independent nominal identity | decide a rule, execute user code, synthesize a source declaration, or ask the host for a width |
...
| `Landin.Stages.Checking` | the three type passes, compile-time-only positional substitution, D136's closed target-independent fixed-expression evaluator, scalar/fixed-array normalization of parameterized aliases, canonical empty-actual nominal assignment for enabled structs, and checking-stage diagnostic order | own a table, a code, execute user code, synthesize a declaration, or choose a target-dependent operator width |
