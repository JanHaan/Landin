...
- The complete parser, containers and hosted application under native LLDB at
  none/off, size/auto and size/all, followed by the full R5.40 debugger contract.
...
retains R5.30's historical selected scope. `--case`, `--profile` and debugger
`--workload` are filtered development feedback, never acceptance.
...
matches retained identity data. Lookup refuses a mismatched map. R5.40's
selected sessions additionally retain all thirteen scalars, source aliases,
