...
DARWIN_TARGET = "arm64-apple-macos26.0.0"
SUPPORTED_TARGETS = (SUPPORTED_TARGET, DARWIN_TARGET)
...
class Policy:
    ...
    def load(cls, path: pathlib.Path) -> "Policy":
        ...
        require(abi.get("target") in SUPPORTED_TARGETS,
                f"policy abi: unsupported target {abi.get('target')!r}")
        expected = {
            "data_model": "lp64",
            "plain_char": "signed",
...
class ClangDriver:
    ...
    def verify_target(self) -> dict[str, str]:
        ...
        require(triple == self.inputs.target,
                f"Clang selected target {triple!r}; required {self.inputs.target!r}")
        ...
        required.update(
            {"__aarch64__": "1", "__APPLE__": "1", "__MACH__": "1"}
            if self.inputs.target == DARWIN_TARGET else
            {"__x86_64__": "1", "__linux__": "1", "__ELF__": "1"})
    ...
    def validate_c(self, adapters: pathlib.Path, staging: pathlib.Path) -> None:
        # SDK 26.5 mixes annotated and unannotated declarations in _stdlib.h.
        # -Wsystem-headers exposes its nullability-completeness warnings; they
...
class Generator:
    ...
    def generate(self) -> tuple[str, str, str, dict[str, Any]]:
        ...
        ldn_sections = [
            ...
            "import core/c\n\ncompiler.assert(compiler."
            + ("c_darwin_lp64" if self.policy.abi["target"] == DARWIN_TARGET
               else "c_sysv_lp64") + ")",
        ]
        if optional_blocks:
        ...
        metadata: dict[str, Any] = {
            ...
            "abi": {
                "target": self.policy.abi["target"],
                "calling_convention": ("darwin-aapcs64"
                    if self.policy.abi["target"] == DARWIN_TARGET else "sysv-amd64"),
    ...
    def verify_rendered(self, bindings: str, adapters: str, exports: str,
        ...
        require(("compiler.assert(compiler."
                 + ("c_darwin_lp64" if self.policy.abi["target"] == DARWIN_TARGET
                    else "c_sysv_lp64") + ")") in bindings,
...
def parse_arguments(argv: Sequence[str]) -> ToolInputs:
    ...
    require(args.target in SUPPORTED_TARGETS,
            f"--target must be one of {SUPPORTED_TARGETS!r}")
